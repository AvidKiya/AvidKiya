...
export default function HomePage() {
  ...
  const process = [
    { title: t('کشف مسئله', 'Discovery'), text: t('هدف، مخاطب، محدودیت‌ها و معیار موفقیت را شفاف می‌کنم.', 'Clarify goals, audience, constraints and success metrics.') },
    { title: t('طراحی سیستم', 'System Design'), text: t('معماری، تجربه کاربری و مسیر رشد محصول را قبل از کدنویسی می‌چینم.', 'Design architecture, UX and scale path before coding.') },
    { title: t('تحویل تمیز', 'Clean Delivery'), text: t('کد قابل نگهداری، مستندات، تست پایه و دیپلوی پایدار تحویل می‌دهم.', 'Ship maintainable code, docs, basic tests and stable deployment.') },
  ];

  return (
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-10">
      <section className="grid lg:grid-cols-[1fr_360px] gap-5 md:gap-6 items-stretch mb-8 md:mb-10">
        <GlassCard className="!p-6 md:!p-8 overflow-hidden relative">
          ...
          <div className="relative z-10">
            <div className="mb-4 opacity-95 scale-[0.92] origin-right md:origin-left" dir="ltr">
              <AsciiLogo small />
            </div>
            <div className="inline-flex items-center gap-2 rounded-full border border-emerald/25 bg-emerald/10 px-3 py-1.5 text-[11.5px] text-emerald mb-4">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald animate-pulse" />
              {t('آماده همکاری روی پروژه‌های وب و AI', 'Available for web & AI projects')}
            </div>
            <h1 className="text-[28px] md:text-[44px] font-[850] tracking-[-0.035em] leading-[1.12] max-w-3xl">
              {tf(cms.identity.fullName)}؛
              <span className="gradient-text block mt-1">{tf(cms.identity.title)}</span>
            </h1>
            <p className="text-[14px] md:text-[15px] text-text-2 mt-4 max-w-[680px] leading-8">
            ...
            <div className="flex flex-wrap items-center gap-2.5 mt-6">
              <Link href="/projects" className="glass-btn-primary !py-3 !px-5 text-[13.5px] inline-flex items-center gap-2">
                {t('دیدن نمونه‌کارها','View work')} <ArrowUpRight size={15} />
              </Link>
              <Link href="/contact" className="glass-btn !py-3 !px-5 text-[13.5px] inline-flex items-center gap-2">
                {t('درخواست پروژه','Start a project')} <ArrowLeft size={15} />
              </Link>
              <Link href="/resume" className="text-[12.5px] text-text-3 hover:text-text transition-colors px-2">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-1 gap-3">
          {stats.map((s, i)=>(
            <GlassCard key={s.id} className="!p-4 flex items-center justify-between gap-3">
              <div>
                <div className="text-[24px] md:text-[28px] font-black tracking-tight">{s.value}</div>
                <div className="text-[11.5px] text-text-3 mt-0.5">{tf(s.label)}</div>
              </div>
              <div className="w-10 h-10 rounded-[14px] bg-white/[0.045] flex items-center justify-center text-[18px]">
                {s.icon || ['⚡','✅','🧠','☁️'][i]}
              </div>
            </GlassCard>
          ))}
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-[14px] mb-9 md:mb-11">
        {cards.map((c,i)=> <MenuCard key={c.href} {...c} index={i} />)}
      </section>

      <section className="grid lg:grid-cols-[390px_1fr] gap-5 items-start mb-9 md:mb-11">
        <CalendarWidget />
        <div>
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <Link href="/services" className="text-[12.5px] text-text-3 hover:text-text flex items-center gap-1">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            {process.map((item, i)=>(
              <GlassCard key={item.title} className="!p-4">
                <div className="w-9 h-9 rounded-[12px] bg-primary/10 text-primary flex items-center justify-center text-[13px] font-black mb-3">0{i+1}</div>
          ...
          <GlassCard className="!p-4 mt-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            <Link href="/projects" className="glass-btn !py-2 !px-4 text-[12.5px] shrink-0">{t('دیدن پروژه‌های مستقل','View standalone projects')}</Link>
          </GlassCard>
        </div>
      </section>
      ...
      <section className="grid md:grid-cols-2 gap-4 mb-9">
        <Link href="/tools" className="group block">
          <GlassCard className="!p-5 h-full hover:shadow-glass-lg transition-all">
            ...
            <p className="text-[12.8px] text-text-2 leading-relaxed">{t('ابزارها حالا داخل خود سایت اجرا می‌شوند؛ سریع، بدون ثبت‌نام و مناسب کارهای روزانه توسعه و محتوا.', 'Tools now run directly in the site; fast, no sign-up and useful for daily development/content tasks.')}</p>
          </GlassCard>
        </Link>
